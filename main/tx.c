...
// Cap send_cb wait (µs). HZ=100 → pdMS_TO_TICKS(1..9) is 0; use esp_timer.
// Slightly over one packet period so a late ACK does not stall forever.
...
// ~40 * 2.5 ms ≈ 100 ms of MAC FAIL → tear down (was ~300 ms @ 7.5 ms pkts).
...
static esp_now_rate_config_t peer_rate_cfg = {
    // 12M is more robust than 24M in moderate crowd / multipath; 480 B airtime
    // is tiny vs the 2.5 ms packet period (try 24M only if TX send fails climb).
