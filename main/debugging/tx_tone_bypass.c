...
#ifdef TX_TONE_BYPASS_ADC
static const char *TONE = "TX_TONE";
#define TONE_MODE_HOLD_MS 8000
#endif
 ...
 *        With TX_TONE_BYPASS_ADC: skip ADC, send digital profile tones instead.
 */
static void audio_capture_task(void *pvParameters) {
    ...
    uint32_t tone_clock = 0;
    tone_mode_t tone_mode = TONE_SINE_440;
    TickType_t tone_mode_started = xTaskGetTickCount();
    int64_t tone_next_due_us = 0;
    ESP_LOGI(TONE, "ADC bypass — mode=%s (hold %d ms)",
             tone_mode_name(tone_mode), TONE_MODE_HOLD_MS);
#endif

    while (!audio_task_stop) {
#ifdef TX_TONE_BYPASS_ADC
        TickType_t tone_now = xTaskGetTickCount();
        if ((tone_now - tone_mode_started) >= pdMS_TO_TICKS(TONE_MODE_HOLD_MS)) {
            tone_mode = (tone_mode_t)((tone_mode + 1) % TONE_MODE_COUNT);
            tone_phase = 0.0;
            tone_clock = 0;
            tone_mode_started = tone_now;
...
void app_main(void) {
    ...
    while (1) {
        ...
        case TX_MODE_BOOTING: {
            ...
            cur_mode = TX_MODE_IDLING;
#ifdef TX_TONE_BYPASS_ADC
            ESP_LOGI(TONE, "Skipping PCM1808 I2S — digital tones only.");
#else
