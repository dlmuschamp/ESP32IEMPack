...
// deprecated. setting to 1 to send full signal through
#define VOL_ATTEN 1 // mainly used to volume safely
// before the headphone amp

// HIGH and LOW signals (16 bit ADC) has a max res of
// 32767 but lowering signal amplitude to provide headroom
...
void init_i2s(void) {
    ...
    // Applying the configuration and turning it on.
...
void app_main(void) {
    ...
    printf("Entering Audio Loop with no attenuation...\n");

    int cur_sample = 0;

    while (1) {
        // Calculate the number of samples per cycle and per half-cycle
        int samples_per_cycle = SAMPLE_RATE / TONE_FREQ_HZ;
        int samples_per_half_cycle = samples_per_cycle / 2;

        // Loop Indicies
        int buf_idx = 0;

        // 1. Create a loop to fill exactly the audio frames (remember, 1 frame
        // = Left + Right channel).
        while (buf_idx < DEFAULT_BUF_SIZE) {
            // 2. Generate a square wave: If you are in the first half of the
            // cycle, make the value high. If in the second half, make it low.
            int sig_mode =
                (cur_sample < samples_per_half_cycle) ? HIGH_SIG : LOW_SIG;

            // 3. Multiply raw wave value by your VOLUME_ATTEN macro.
            int final_sig = sig_mode * VOL_ATTEN;

            // 4. Assign this final value to both the left and right slots in
            // the array.
        ...
        i2s_channel_write(tx_chan, sample_buffer, sizeof(sample_buffer),
