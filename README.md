...
| Radio | ESP32-WROOM-32U + **external 2.4 GHz U.FL pigtail antenna** | On-module PCB antenna (that is the 32E / 32D) |
...
The **32U** module has a U.FL connector and **no PCB antenna**. Wi-Fi / ESP-NOW will be dead without a **2.4 GHz antenna on a U.FL / IPEX pigtail** mated to the module. Do not leave this off the BOM. The schematic symbol for that pigtail (`ANT1`) lives on the follow-up fixes branch so you can merge this folder-sync PR first.
...
The PCM5102 datasheet analog network for a ~10 kΩ load is:

`OUTL/OUTR → 470 Ω series → PRE_POT`, with **2.2 nF from that node to GND**.

The MAX97220A input impedance is also about **10 kΩ**, so that filter is the right starting point. The 1 kΩ / 10 Ω pad on the volume sheet was an experiment, not the datasheet circuit.

MAX97220A voltage gain is **fixed +3.5 dB**. The 10 kΩ resistors around the amp are the single-ended input network, not an IC “gain pin.” Leave them unpopulated on a hand-built proto so you can try values. Populate them (start 10 kΩ) only if you order a fully assembled board. Use a MAX97220**D** if you want resistor-programmed gain.

### Hand assembly

Use **0805** for every resistor and capacitor you will solder yourself, including 10 / 22 / 33 µF. 0603 is OK if you are already comfortable. Do not use 0402 or 0201. KiCad’s default `C_0201_0603Metric` / `R_0201_0603Metric` names look like “0603” but they are imperial **0201**.
...
- [ ] Fit the U.FL antenna; confirm ESP-NOW range
